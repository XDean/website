...
export const Home = (props: Props) => {
    ...
    return (
      <div className={'h-full w-full relative flex flex-col items-center justify-center'}>
        ...
        {img.title && (
          <div ref={ref}
               className={'absolute z-50 left-2 bottom-2'}>
            <a
              className={'hidden md:block px-1 text-sm opacity-60 text-gray-700 bg-gray-300 ' +
        ...
        <div className={clsx(
          'flex flex-col items-center justify-center relative transition duration-700',
          'p-4 rounded drop-shadow-2xl backdrop-opacity-50 backdrop-blur-3xl bg-white/10',
          hovered && 'opacity-0',
        )}>
          <div className={css.logoContainer}>
            <Image src={xdean}
                   className={clsx(css.logo, css.xdean)}/>
            <Image src={xd}
                   className={clsx(css.logo, css.xd)}/>
          </div>
          ...
          <div className={'flex flex-col lg:flex-row items-center space-y-2 lg:space-y-0 lg:space-x-4 text-3xl'}>
            <Link link={'#about'}
                  text={'关于我'}/>
            <Dot/>
            <Link link={'https://blog.xdean.cn'}
                  targetBlank
                  text={'博客'}/>
            <Dot/>
            <Link link={'https://tutorial.xdean.cn'}
                  targetBlank
                  text={'教程'}/>
            <Dot/>
            <Link link={'https://tool.xdean.cn'}
                  targetBlank
                  text={'百宝箱'}/>
          </div>
        </div>
